var tabla_seguimiento = $('.datatables-seguimiento'),
 //basic_used_cupons = $('.datatables-basic_notificaciones_usados'),
   dt_date_table = $('.dt-date'),
   assetPath = '../../dataPY4/';
function cargaTablSeguimientoNotif(editada) {

  let valor_seguimiento1 = $('#array_seguimiento_notificaciones').val()
 ...
 if ($('body').attr('data-framework') === 'laravel') {
  ...
  if (tabla_seguimiento.length) {
    console.log(array_seguimiento)
    var dt_basic_notificaciones = tabla_seguimiento.DataTable({
      data: array_seguimiento,
      columns: [
        { data: 'notif1_2.id' },
        { data: 'notif1_2.id' },
        { data: 'notif1_2.id' }, // used for sorting so will hide this column
        { data: 'notif1_2.id' },
        { data: 'notif1_2.' },
        {   // Actions
          targets: -1,
    ...
    console.log('entro aqui')
        $.ajax({
          ...
          success: function (data, textStatus, jqXHR) {
            ...
            $('.datatables-seguimiento').dataTable().fnDestroy();
             $('.datatables-seguimiento').empty();
            $('.datatables-seguimiento').html(`<thead>
            <tr>
              <th>id</th>
              <th>Tipo</th>
              <th>Estado</th>
